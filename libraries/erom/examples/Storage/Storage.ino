...
erom::Entry<int>   ei = storage.issue<int>();
erom::Entry<long>  el = storage.issue<long>();
erom::Entry<float> ef = storage.issue<float>();
...
void load_values() { ei.load(); el.load(); ef.load(); }
...
void print_help() {
  ...
  Serial.println(" U - update EEPROM memory (write changes only)");
